        ...
        <section class="content connectedSortable">
            <div class="row">
                ...
                <div class="col-md-12">
                        ...
                        <div class="box-body">
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-md-6">
                                    <button type="button" onclick="cetak_arsip('Excel')" class="btn btn-info btn-block"><i class="fa fa-print"></i> Excel</button>
                                </div>
                                <div class="col-sm-12 col-md-6">
                                    <button type="button" onclick="cetak_arsip('PDF')" class="btn btn-info btn-block" style="margin-right: 5px"><i class="fa fa-print"></i> PDF</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="box box-brown">
                        <div class="box-header">
                            <h3 class="box-title">Arsip KP</h3>
...
<script type="text/javascript">
    ...
    document.getElementById('d_arsip_kp').setAttribute("class", "active");
    $(function () {
        var dt_table = $("#example2").DataTable({
            ...
            "paging": true,
            "ajax": {
                "url": '<?= base_url('Admin/arsip_kp_data');?>',
                "type": "POST",
                "data": function (d) {
                    loading_page_start();
                    d.tahun = $('#tahun').val();
            ...
            "columns": [
                { "render": function ( data, type, row )
                    {
                        return '<center><input type="checkbox" name="tandai[]" value="'+row.id_kp+'"></center>';
                    }
                },
                ...
                { "render": function ( data, type, row )
                    {
                        return '<a href="javascript:void()" onclick="read(\''+row.id_kp+'\')">'+row.id_kp+'</a>'
                    }
                },
    ...
    function page(str)
    {
        window.location = '<?php echo base_url('Admin/Arsip_KP/')?>'+str;
    }
    function read(str)
    {
        loading_page_end();
    ...
    {
        window.open("<?php echo base_url('Admin/download_srt_pers')?>"+"/"+str, '_blank');
    }
    function cetak_arsip(str)
    {
        window.open("<?= base_url('Admin/arsip_kp_rekap');?>?type="+str+"&tahun="+$('#tahun').val(), '_blank');
    }
    function check_all() {
    ...
    function execute_tandai(str) {
        ...
        for (var i = 0; i < ttl_tandai; i++) {
            if (tandai[i].checked){
                data.append("id_kp[]", tandai[i].value);
            } else {
                ttl_tandai_check++;
            }
        }
            ...
            function(){
                setTimeout(function(){
                    $.ajax({
                        ...
                        contentType: false, url: "<?php echo base_url('Admin/arsip_kp_proses')?>",
                        success: function(data) {
                            if (data==1) {
                                swal("Sukses", "", "success");
                                cari();
