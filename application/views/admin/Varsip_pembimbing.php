    ...
    <div class="content-wrapper">
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li class="active">Arsip Pembimbing</li>
            </ol>
        </section>
        <section class="content connectedSortable">
            <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    <div class="box box-brown">
                        <div class="box-header">
                            <h3 class="box-title">Arsip Pembimbing</h3>
...
<script type="text/javascript">
    ...
    document.getElementById('d_arsip_pembimbing').setAttribute("class", "active");
    $(function () {
        var dt_table = $("#example2").DataTable({
            "processing": true,
            "serverSide": true,
            "searching": true,
            "deferRender": true, "responsive": true,
            "pagingType": "simple_numbers",
            "lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
            "language" : {"url" : "https://cdn.datatables.net/plug-ins/1.10.19/i18n/Indonesian.json"},
            "paging": true,
            "ajax": {
                "url": '<?= base_url('Admin/arsip_pembimbing_json');?>',
                "type": "POST",
                "data": function (d) {
                    loading_page_start();
                    d.tahun = $('#tahun').val();
            ...
            "columns": [
                { "render": function ( data, type, row )
                    {
                        return '<center><input type="checkbox" name="tandai[]" value="'+row.id_pembimbing_mahasiswa+'/'+row.nama_mahasiswa+'/'+row.nama_dosen+'"></center>';
                    }
                },
                ...
                { "render": function ( data, type, row )
                    {
                        return '<img style="width:100%;"src="'+row.foto_dosen+'" class="profile-user-img img-responsive img-circle" alt="Dosen Image">';
                    }
                },
                { "render": function ( data, type, row )
                    {
                        return '<a href="javascript:void();" onclick="dtl_dsn(\''+row.id_dosen+'\');">'+row.nama_dosen+'</a>';
                    }
                },
                { "render": function ( data, type, row )
                    {
                        return '<img style="width:100%;"src="'+row.foto_mahasiswa+'" class="profile-user-img img-responsive img-circle" alt="Dosen Mahasiswa">';
                    }
                },
                { "render": function ( data, type, row )
                    {
                        return '<a href="javascript:void();" onclick="dtl_mhs(\''+row.id_mahasiswa+'\');">'+row.nama_mahasiswa+'</a>';
                    }
                },
                {"data": "nama_perusahaan"},
                { "render": function ( data, type, row )
                    {
                        return '<a href="javascript:void();" onclick="detail(\''+row.id_kp+'\');">'+row.id_kp+'</a>';
                    }
                },
                {"data": "nama_laporan"},
                { "render": function ( data, type, row )
                    {
                        var view = '';
                        if (row.stt === 1){
                            view = '<button type="button" class="btn btn-success col-md-12">Laporan Selesai</button>';
                        } else {
                            view = '<button type="button" class="btn btn-danger col-md-12"> Laporan Belum Selesai</button>';
                        }
                        return view;
                    }
                },
    ...
    function download_arsip(type) {
        window.open("<?= base_url('Admin/arsip_pembimbing_cetak?type=')?>"+type+"&tahun="+$('#tahun').val(), '_blank');
    }
    function check_all() {
    ...
    function execute_tandai(str) {
        ...
        for (var i = 0; i < ttl_tandai; i++) {
            if (tandai[i].checked){
                data.append("id_pembimbing[]", tandai[i].value);
            } else {
                ttl_tandai_check++;
            }
        }
            ...
            function(){
                setTimeout(function(){
                    $.ajax({
                        ...
                        contentType: false, url: "<?php echo base_url('Admin/arsip_pembimbing_proses')?>",
                        success: function(data) {
                            if (data==1) {
                                swal("Sukses", "", "success");
                                cari();
